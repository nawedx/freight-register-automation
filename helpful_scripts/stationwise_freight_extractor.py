...
df = pd.read_csv('MisOwtdFrgtRgtr.csv', skiprows=2)
...
#print(df)
df2 = pd.read_table('TrfcErngLdng.xls', skiprows=2)
...
dict2 = {'ACTR':'TLHR', 'SBCT':'TLHR', 'BCMT':'TLHR', 'MGCT':'TLHR', 'DBCS':'TLHR', 'LMGT':'TLHR', 'BGMT':'TLHR', 'CBSP':'PRDP', 'PPAP':'PRDP', 'PMIP':'PRDP', 'PPTP':'PRDP'}
...
df5 = pd.DataFrame(columns=['STATION_FROM', 'CMDT', 'TOTAL_WEIGHT_CHRG', 'TOTAL_FRGT'])
...
for i in stnList:
	...
	for j in cmdtList:
		...
		totalFreight = df1['TOTAL_FRGT'].sum()
		#print(i, j, totalCharge, totalFreight)
		df5 = df5.append({'STATION_FROM':i, 'CMDT':j, 'TOTAL_WEIGHT_CHRG':totalCharge, 'TOTAL_FRGT':totalFreight}, ignore_index=True)
		print(df5)
